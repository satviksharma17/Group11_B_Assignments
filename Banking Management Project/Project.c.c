...
int account()
{
    ...
    if(choice==1){
        ...
        {
            printf("\n\t\tPLEASE ENTER CORRECT MOBILE NUMBER(10 digits Only) :");
             scanf("%s",d.mobile);
         }
        ...
        while(fscanf(q,"%d %s %d %f %s %c\n",&d1.number,d1.name,&d1.age,&d1.amount,d1.mobile,&d1.l)!=EOF)
        {
            if(d1.number==an && choice==1)
            {
               ...
               printf("Enter the ammount:");
                ...
                else { d1.amount=d1.amount-money;
                money=d1.amount;
                fprintf(w,"%d %s %d %f %s %c\n",d1.number,d1.name,d1.age,money,d1.mobile,d1.l);
...
void apply()
{
    ...
    if(choice==1)
    {
        ...
        scanf("%d",&choice);
         printf("\n");
        if(choice==1)
            printf("VISA CARD WILL BE DELIEVED TO YOU");
